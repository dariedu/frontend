...
const delivery1: IDelivery = {
  ...
  price: 5,
   ...
   address: "поселение Внуковсекое, ул. Авиаконстраукора Петькина, д.15 к1. строение 15 кв. 222",
   link: "null",
  subway: "Белорусская",
  media_files: null,
...
let deliveryDate = new Date(delivery.date)
  ...
  return (
    <>
      ...
      ? <button className='btn-S-GreenDefault' onClick={(e)=>{e.preventDefault()}}>Ближайшая</button>
          : <button className='btn-S-GreenDefault' onClick={(e) => { e.preventDefault() }}>Активная</button>
        }
        ...
        <div className='flex w-fit'>
          ...
          <div className='flex flex-col justify-start items-start pl-2 w-max-[290px] pt-[20px]'>
            <h1 className='font-gerbera-h3 text-light-gray-8'>Ст. {delivery.location.subway}</h1>
            <p className='font-gerbera-sub1 tetx-light-gray-5 text-left h-fit w-max-[290px]'>{ delivery.location.address}</p>
          </div> 
        </div>
        {/* /////////////////////// */}
        {volunteer
          ? (<div className="flex justify-between items-center mt-[14px]">
            ...
            : (<div className="flex justify-between items-center mt-[20px]">
              ...
              <p className="font-gerbera-h3 text-light-gray-8">
                {delivery.volunteers_taken == 0 ? 'Пока никто' : `${delivery.volunteers_taken + " " + getPersonCorrectEndingName(delivery.volunteers_taken)}`}
        ...
        {volunteer ?
          (<div className='w-[330px] h-[67px] bg-light-gray-1 rounded-2xl mt-[20px] flex items-center justify-between px-4'>
            <div className='flex'>
           <img className='h-[32px] w-[32px] rounded-full' src={delivery.curator.avatar} />
              <div className='felx flex-col justify-center items-start ml-4'>
                <h1 className='font-gerbera-h3 text-light-gray-8-text text-start'>{delivery.curator.name}</h1>
                 <p className='font-gerbera-sub2 text-light-gray-2 text-start'>Куратор</p>
              </div>
             
            </div>
            <a href={"https://t.me/" + curatorTelegramNik} target='_blank'><img src='../src/assets/icons/small_sms.svg' className='w-[36px] h-[35px]'/></a>
              
          </div>
          ) : ""}
        <div className='w-[330px] h-[67px] bg-light-gray-1 rounded-2xl mt-[20px] flex flex-col items-start justify-center px-4'>
          <h1 className='font-gerbera-h3 text-light-gray-8-text'>Пункты маршрута</h1>
          <p className='font-gerbera-sub1 text-light-gray-5 mt-[6px]'>Пункт выдачи откуда забирать </p>
        </div>
        {booked ? (
      <button className='btn-B-GrayDefault mt-[20px]' onClick={(e)=>{e.preventDefault()}}>Отказаться</button>
        ) : (
            <button className='btn-B-GreenDefault mt-[20px]' onClick={(e)=>{e.preventDefault()}} >Записаться</button>
        )}
