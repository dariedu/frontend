...
const RouteSheet: React.FC<IProps> = ({ routeS, index, setOpenVolunteerLists, listOfVolunteers, openVolunteerLists, changeListOfVolunteers,
  onVolunteerAssign, deliveryId, assignVolunteerFail, setAssignVolunteerFail, assignVolunteerSuccess, setAssignVolunteerSuccess, openRouteSheets,
  setOpenRouteSheets, myPhotoReports
})=>{
...
const [thisRouteMyPhotoReports, setThisRouteMyPhotoReports] = useState<TServerResponsePhotoReport[]>([]);
   ...
   function filterPhotoReports() {
      if (myPhotoReports && myPhotoReports.length > 0 && routeS.address.length > 0) {
     let filtered = myPhotoReports
            .filter(report => {
              if (report.route_sheet_id == routeS.address[0].route_sheet) return report;
            });
    ...
    useEffect(() => {
      if (routeS.address.length > 0 && thisRouteMyPhotoReports.length == routeS.address.length) {
        setFinished(true)
  ...
  return (
    <div key={routeS.id}
      ...
      {finished && <div className="font-gerbera-h3 text-light-gray-8-text dark:text-light-gray-white self-center text-center mt-2 " >
