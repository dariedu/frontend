...
const defaultEvent: IPromotion = {
  ...
  city: 'Москва',
  users: [0],
  ticket: "Мск, ул. Бобруйская д.6 к.2  вход с правой стороны от здания. Кодовое слово \"Добро\"",
...
const DetailedInfo: React.FC<IDefaultInfoProps> = ({ promotion=defaultEvent }) => {
  return (
    <div className='w-[360px] flex flex-col h-fit rounded-t-2xl px-4 pt-[41px] pb-8' >
      ...
      <div className="bg-light-gray-1 rounded-2xl flex flex-col justify-between items-start p-4 mt-[14px]">
        ...
        <p className="w-[296px] h-fit font-gerbera-sub1 text-start mt-[10px]">{promotion.ticket}</p>
      </div>
      ...
      <div className='flex justify-between items-center mt-[14px]'>
        <div className='bg-light-gray-1 rounded-2xl flex flex-col justify-between items-start w-40 h-[62px] p-[12px]'>
          <p className='font-gerbera-sub2 text-light-gray-black '>Время начала</p>
          <p className='font-gerbera-h3 text-light-gray-black'>{promotion.start_date.getHours()}:{promotion.start_date.getMinutes()}</p>
        </div>
        <div className='bg-light-gray-1 rounded-2xl flex flex-col justify-between items-start w-40 h-[62px] p-[12px]'>
          <p className='font-gerbera-sub2 text-light-gray-black '>Списание баллов</p>
          <p className='font-gerbera-h3 text-light-brand-green '>{promotion.price} балла(-ов)</p>
        </div>
      </div>
      <div className='w-[328px] h-fit p-4 bg-light-gray-1 rounded-2xl mt-[14px] flex flex-col justify-center items-start'>
        ...
        <p className='font-gerbera-sub1 text-light-gray-4 h-full text-start mt-[10px]' >{promotion.description}</p>
      </div>
      {(promotion.picture.length > 0) ? (
      <img className='w-[314px] h-[205px] rounded-2xl mt-[14px] self-center'  src={promotion.picture} />
      ) : ""}
      <div className='flex justify-between items-center mt-[14px]'>
        <button onClick={(e)=>{e.preventDefault()}} className='btn-M-GreenDefault'>Забронировать</button>
        <button onClick={(e)=>{e.preventDefault()}} className='btn-M-WhiteDefault'>Отменить</button>
      </div>
    </div>
