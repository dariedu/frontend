...
export const getAllPromotions = async (
  ...
  try {
    ...
    response.data.forEach(i => {result.push(parseObject(i))});
    return result;
  } catch (err: any){
...
export const postPromotionRedeem = async(promotionId: number): Promise<IPromotion> => {
  try {
    const response: AxiosResponse<string> = await axios({
...
export const postPromotionCancel = async(promotionId: number): Promise<IPromotion> => {
  try {
    const response: AxiosResponse<string> = await axios({
