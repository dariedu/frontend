...
const InputOptions: React.FC<IInputOptionsProps> = ({
  ...
  return (
    <div
      className={
        clicked
          ? 'bg-light-gray-1 rounded-t-2xl z-100 '
          : 'bg-light-gray-1 rounded-2xl'
      }
      onClick={e => {
        e.stopPropagation();
    ...
    >
      ...
      <div
        className={
          clicked
            ? 'bg-light-gray-1 flex flex-col h-[108px] rounded-b-2xl overflow-y-scroll w-[328px] text-left absolute z-200'
            : 'absolute hidden'
        }
      >
        {options
          ...
          .map((opt: [number, string], i: number) => {
            return (
              <li
                ...
                className={
                  choiceMade == opt[0]
                    ? 'bgImageInputOptions pl-8 list-none py-[16px] rounded-2xl text-light-gray-8-text font-gerbera-h3'
                    : 'px-8 list-none py-[16px] w-[300px] rounded-2xl text-light-gray-8-text font-gerbera-h3'
                }
