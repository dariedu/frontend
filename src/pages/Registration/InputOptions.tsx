...
const InputOptions: React.FC<IInputOptionsProps> = ({
  ...
  return style ? (
    <div
      className={
        clicked
          ? 'bg-light-gray-1 dark:bg-light-gray-6 rounded-t-2xl z-100 relative'
          : 'bg-light-gray-1 dark:bg-light-gray-6 rounded-2xl relative'
      }
      onClick={e => {
        e.stopPropagation();
    ...
    >
      ...
      <div
        className={
          clicked
            ? 'bg-light-gray-1 w-full dark:bg-light-gray-6 flex flex-col h-[108px] rounded-b-2xl overflow-y-scroll text-left z-200 absolute'
            : ' hidden'
        }
      >
        {options
          ...
          .map((opt: [number, string] | string[], i: number) => {
            return (
              <li
                ...
                className={
                  choiceMade == opt[0]
                    ? 'bgImageInputOptions pl-8 list-none py-[16px] rounded-2xl text-light-gray-8-text dark:text-light-gray-white font-gerbera-h3'
                    : 'px-8 list-none py-[16px]  rounded-2xl text-light-gray-8-text dark:text-light-gray-white font-gerbera-h3'
                }
