...
//   function requestEachMyDelivery(deliveries: TCuratorDelivery[], setStateAction:React.Dispatch<React.SetStateAction<IDelivery[]>> ) { 
//     if (token) {
    ...
    <div className="flex-col min-h-[80vh] bg-light-gray-1  dark:bg-light-gray-black ">
      ...
      {curatorActiveDeliveries && curatorActiveDeliveries.length >0 ? (
        curatorActiveDeliveries.sort((a, b) => { return +(new Date(a.id_delivery)) - +(new Date(b.id_delivery)) }).map((del, index) => {
          // let curatorDel = curatorActiveDeliveries.find(i => i.id_delivery == del.id)
          return (<div key={index}>
